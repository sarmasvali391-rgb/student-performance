...
function showCategory(category, btn) {
  ...
  if (category === 'degree') {
    ...
    if (first) { document.querySelectorAll('.sub-btn').forEach(b => b.classList.remove('active')); first.classList.add('active'); }
  }
  if (category === 'pg') {
...
function emailPUCReport() {
  ...
  const subject = `Student Performance Report - ${name}`;
  const body = `
