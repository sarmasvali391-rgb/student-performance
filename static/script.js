...
function loadBCASingleSubjects() {
  ...
  let html = '';
  bcaSubjects[sem].forEach(sub => {
    const id = sub.replace(/\s+/g,'_').toLowerCase();
    html += `<div class="form-group"><label>${sub}</label><input type="number" id="bca_single_${id}" placeholder="e.g. 75"></div>`;
  });
...
function loadBCAAllSubjects() {
  ...
  Object.entries(bcaSubjects).forEach(([sem, subjects]) => {
    ...
    subjects.forEach(sub => {
      const id = sub.replace(/\s+/g,'_').toLowerCase();
      html += `<div class="form-group"><label>${sub}</label><input type="number" id="bca_all_${sem.replace(/\s+/g,'_')}_${id}" placeholder="e.g. 75"></div>`;
    });
  });
...
function loadBBASingleSubjects() {
  ...
  let html = '';
  bbaSubjects[sem].forEach(sub => {
    const id = sub.replace(/\s+/g,'_').toLowerCase();
    html += `<div class="form-group"><label>${sub}</label><input type="number" id="bba_single_${id}" placeholder="e.g. 75"></div>`;
  });
...
function loadBBAAllSubjects() {
  ...
  Object.entries(bbaSubjects).forEach(([sem, subjects]) => {
    ...
    subjects.forEach(sub => {
      const id = sub.replace(/\s+/g,'_').toLowerCase();
      html += `<div class="form-group"><label>${sub}</label><input type="number" id="bba_all_${sem.replace(/\s+/g,'_')}_${id}" placeholder="e.g. 75"></div>`;
    });
  });
...
function loadBComSingleSubjects() {
  ...
  let html = '';
  bcomSubjects[sem].forEach(sub => {
    const id = sub.replace(/\s+/g,'_').toLowerCase();
    html += `<div class="form-group"><label>${sub}</label><input type="number" id="bcom_single_${id}" placeholder="e.g. 75"></div>`;
  });
...
function loadBComAllSubjects() {
  ...
  Object.entries(bcomSubjects).forEach(([sem, subjects]) => {
    ...
    subjects.forEach(sub => {
      const id = sub.replace(/\s+/g,'_').toLowerCase();
      html += `<div class="form-group"><label>${sub}</label><input type="number" id="bcom_all_${sem.replace(/\s+/g,'_')}_${id}" placeholder="e.g. 75"></div>`;
    });
  });
...
function loadMCASingleSubjects() {
  ...
  let html = '';
  mcaSubjects[sem].forEach(sub => {
    const id = sub.replace(/\s+/g,'_').toLowerCase();
    html += `<div class="form-group"><label>${sub}</label><input type="number" id="mca_single_${id}" placeholder="e.g. 75"></div>`;
  });
...
function loadMCAAllSubjects() {
  ...
  Object.entries(mcaSubjects).forEach(([sem, subjects]) => {
    ...
    subjects.forEach(sub => {
      const id = sub.replace(/\s+/g,'_').toLowerCase();
      html += `<div class="form-group"><label>${sub}</label><input type="number" id="mca_all_${sem.replace(/\s+/g,'_')}_${id}" placeholder="e.g. 75"></div>`;
    });
  });
...
function loadMBASingleSubjects() {
  ...
  let html = '';
  mbaSubjects[sem].forEach(sub => {
    const id = sub.replace(/\s+/g,'_').toLowerCase();
    html += `<div class="form-group"><label>${sub}</label><input type="number" id="mba_single_${id}" placeholder="e.g. 75"></div>`;
  });
...
function loadMBAAllSubjects() {
  ...
  Object.entries(mbaSubjects).forEach(([sem, subjects]) => {
    ...
    subjects.forEach(sub => {
      const id = sub.replace(/\s+/g,'_').toLowerCase();
      html += `<div class="form-group"><label>${sub}</label><input type="number" id="mba_all_${sem.replace(/\s+/g,'_')}_${id}" placeholder="e.g. 75"></div>`;
    });
  });
...
async function analyzePUC() {
  ...
  try {
    const response = await fetch('/analyze_puc', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(payload) });
...
async function analyzeBCASingle() {
  ...
  const payload = { name, roll, college, attendance, study_hours, gender: document.getElementById('bca_gender').value, semester, subjects };
  try {
    const response = await fetch('/analyze_bca_single', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(payload) });
    const data = await response.json();
    ...
    if (window.bcaBarChart) window.bcaBarChart.destroy();
    if (window.bcaRadarChart) window.bcaRadarChart.destroy();
    window.bcaBarChart = new Chart(document.getElementById('bca_barChart'), {
      type: 'bar',
      data: { labels: data.subject_names, datasets: [{ label: 'Marks out of 100', data: data.subject_marks, backgroundColor: ['#4f46e5','#06b6d4','#10b981','#f59e0b','#ef4444'], borderRadius: 8 }] },
      options: { responsive: true, scales: { y: { beginAtZero: true, max: 100 } } }
    });
    window.bcaRadarChart = new Chart(document.getElementById('bca_radarChart'), {
      type: 'radar',
      data: { labels: data.subject_names, datasets: [{ label: 'Marks', data: data.subject_marks, backgroundColor: 'rgba(79,70,229,0.2)', borderColor: '#4f46e5', pointBackgroundColor: '#4f46e5', borderWidth: 2 }] },
      options: { responsive: true, scales: { r: { beginAtZero: true, max: 100 } } }
    });
...
async function analyzeBCAAll() {
  ...
  const payload = { name, roll, college, attendance, study_hours, gender: document.getElementById('bca_gender').value, semesters };
  try {
    const response = await fetch('/analyze_bca_all', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(payload) });
    const data = await response.json();
    ...
    data.sem_results.forEach(sem => {
      semHtml += `<div class="sem-card"><h3>${sem.semester}</h3><p>Total: ${sem.total}/500</p><p>Percentage: ${sem.percentage}%</p><p>CGPA: ${sem.cgpa}</p><p>Status: ${sem.status.includes('PASS') ? '✅ PASS' : '❌ FAIL'}</p>${sem.weak_subjects.length > 0 ? '<p>Weak: ' + sem.weak_subjects.join(', ') + '</p>' : '<p>No weak subjects ✅</p>'}</div>`;
    });
    semHtml += '</div>';
    ...
    if (window.bcaAllBarChart) window.bcaAllBarChart.destroy();
    if (window.bcaAllRadarChart) window.bcaAllRadarChart.destroy();
    window.bcaAllBarChart = new Chart(document.getElementById('bca_all_barChart'), {
      type: 'bar',
    ...
    window.bcaAllRadarChart = new Chart(document.getElementById('bca_all_radarChart'), {
      type: 'radar',
...
async function analyzeBBASingle() {
  ...
  const payload = { name, roll, college, attendance, study_hours, gender: document.getElementById('bba_gender').value, semester, subjects };
  try {
    const response = await fetch('/analyze_bba_single', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(payload) });
    const data = await response.json();
...
async function analyzeBBAAll() {
  ...
  const payload = { name, roll, college, attendance, study_hours, gender: document.getElementById('bba_gender').value, semesters };
  try {
    const response = await fetch('/analyze_bba_all', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(payload) });
    const data = await response.json();
...
async function analyzeBComSingle() {
  ...
  const payload = { name, roll, college, attendance, study_hours, gender: document.getElementById('bcom_gender').value, semester, subjects };
  try {
    const response = await fetch('/analyze_bcom_single', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(payload) });
    const data = await response.json();
...
async function analyzeBComAll() {
  ...
  const payload = { name, roll, college, attendance, study_hours, gender: document.getElementById('bcom_gender').value, semesters };
  try {
    const response = await fetch('/analyze_bcom_all', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(payload) });
    const data = await response.json();
...
async function analyzeBScSingle() {
  ...
  const payload = { name, roll, college, attendance, study_hours, gender: document.getElementById('bsc_gender').value, semester, subjects };
  try {
    const response = await fetch('/analyze_bsc_single', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(payload) });
    const data = await response.json();
...
async function analyzeBScAll() {
  ...
  const payload = { name, roll, college, attendance, study_hours, gender: document.getElementById('bsc_gender').value, semesters };
  try {
    const response = await fetch('/analyze_bsc_all', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(payload) });
    const data = await response.json();
...
async function analyzeMCASingle() {
  ...
  const payload = { name, roll, college, attendance, study_hours, gender: document.getElementById('mca_gender').value, semester, subjects };
  try {
    const response = await fetch('/analyze_mca_single', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(payload) });
    const data = await response.json();
    ...
    if (window.mcaBarChart) window.mcaBarChart.destroy();
    if (window.mcaRadarChart) window.mcaRadarChart.destroy();
...
async function analyzeMCAAll() {
  ...
  const payload = { name, roll, college, attendance, study_hours, gender: document.getElementById('mca_gender').value, semesters };
  try {
    const response = await fetch('/analyze_mca_all', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(payload) });
    const data = await response.json();
    ...
    if (window.mcaAllBarChart) window.mcaAllBarChart.destroy();
    if (window.mcaAllRadarChart) window.mcaAllRadarChart.destroy();
    window.mcaAllBarChart = new Chart(document.getElementById('mca_all_barChart'), {
      type: 'bar',
      data: { labels: data.sem_results.map(s => s.semester), datasets: [{ label: 'CGPA', data: data.sem_results.map(s => s.cgpa), backgroundColor: ['#4f46e5','#06b6d4','#10b981','#f59e0b'], borderRadius: 8 }] },
      options: { responsive: true, scales: { y: { beginAtZero: true, max: 10 } } }
    });
...
async function analyzeMBASingle() {
  ...
  const payload = { name, roll, college, attendance, study_hours, gender: document.getElementById('mba_gender').value, semester, subjects };
  try {
    const response = await fetch('/analyze_mba_single', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(payload) });
    const data = await response.json();
...
async function analyzeMBAAll() {
  ...
  const payload = { name, roll, college, attendance, study_hours, gender: document.getElementById('mba_gender').value, semesters };
  try {
    const response = await fetch('/analyze_mba_all', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(payload) });
    const data = await response.json();
...
// ── EMAIL ──
function emailPUCReport() {
  const name = document.getElementById('puc_res_name').textContent;
  const total = document.getElementById('puc_res_total').textContent;
  const percentage = document.getElementById('puc_res_pct').textContent;
  const grade = document.getElementById('puc_res_grade').textContent;
  ...
  const weak = document.getElementById('puc_res_weak').textContent;
  const subject = encodeURIComponent(`Student Performance Report - ${name}`);
  const body = encodeURIComponent(`Student Performance Report\n==========================\nStudent Name: ${name}\nTotal Marks: ${total}\nPercentage: ${percentage}\nGrade: ${grade}\nStatus: ${status}\nRisk Level: ${risk}\nWeak Subjects: ${weak}\n\nGenerated by Student Performance Analysis System`);
...
function savePUCPDF() {
  ...
  const status = document.getElementById('puc_res_status').textContent;
  const risk = document.getElementById('puc_res_risk').textContent;
  const printContents = `<html><head><title>PUC Report</title><meta charset="UTF-8"><style>body{font-family:Arial,sans-serif;padding:30px;color:#333}.header{background:#000;color:white;padding:20px;text-align:center;margin-bottom:30px;border-radius:10px}h1{margin:0;font-size:1.5rem}h2{color:#1a73e8;border-bottom:2px solid #1a73e8;padding-bottom:8px;margin-top:30px}table{width:100%;border-collapse:collapse;margin:15px 0}td,th{border:1px solid #ddd;padding:12px 16px;text-align:left}th{background:#1a73e8;color:white}tr:nth-child(even){background:#f5f5f5}.pass{color:green;font-weight:bold}.fail{color:red;font-weight:bold}.footer{text-align:center;color:#999;font-size:0.8rem;margin-top:30px}</style></head><body><div class="header"><h1>📊 PUC Performance Report</h1></div><h2>Student Details</h2><table><tr><th>Field</th><th>Details</th></tr><tr><td>Student Name</td><td>${name}</td></tr><tr><td>Total Marks</td><td>${document.getElementById('puc_res_total').textContent}</td></tr><tr><td>Percentage</td><td>${document.getElementById('puc_res_pct').textContent}</td></tr><tr><td>Grade</td><td>${document.getElementById('puc_res_grade').textContent}</td></tr><tr><td>Status</td><td class="${status.includes('PASS')?'pass':'fail'}">${status}</td></tr><tr><td>Risk Level</td><td>${risk}</td></tr><tr><td>Weak Subjects</td><td>${document.getElementById('puc_res_weak').textContent}</td></tr></table><h2>Subject Marks</h2><table><tr><th>Subject</th><th>Marks</th><th>Status</th></tr><tr><td>Mathematics</td><td>${document.getElementById('puc_math').value}</td><td class="${document.getElementById('puc_math').value>=35?'pass':'fail'}">${document.getElementById('puc_math').value>=35?'✅ PASS':'❌ FAIL'}</td></tr><tr><td>Science</td><td>${document.getElementById('puc_science').value}</td><td class="${document.getElementById('puc_science').value>=35?'pass':'fail'}">${document.getElementById('puc_science').value>=35?'✅ PASS':'❌ FAIL'}</td></tr><tr><td>English</td><td>${document.getElementById('puc_english').value}</td><td class="${document.getElementById('puc_english').value>=35?'pass':'fail'}">${document.getElementById('puc_english').value>=35?'✅ PASS':'❌ FAIL'}</td></tr><tr><td>Kannada</td><td>${document.getElementById('puc_kannada').value}</td><td class="${document.getElementById('puc_kannada').value>=35?'pass':'fail'}">${document.getElementById('puc_kannada').value>=35?'✅ PASS':'❌ FAIL'}</td></tr><tr><td>Social Science</td><td>${document.getElementById('puc_social').value}</td><td class="${document.getElementById('puc_social').value>=35?'pass':'fail'}">${document.getElementById('puc_social').value>=35?'✅ PASS':'❌ FAIL'}</td></tr></table><div class="footer"><p>Generated by Student Performance Analysis System | ${new Date().toLocaleDateString()}</p></div></body></html>`;
...
function saveBCASinglePDF() {
  ...
  printPDF('BCA', name, semester, status, risk, subjectRows, document.getElementById('bca_res_total').textContent, document.getElementById('bca_res_pct').textContent, document.getElementById('bca_res_cgpa').textContent, document.getElementById('bca_roll').value, document.getElementById('bca_college').value, document.getElementById('bca_res_weak').textContent);
}

function saveBCAAllPDF() {
  printAllPDF('BCA', document.getElementById('bca_all_res_name').textContent, document.getElementById('bca_roll').value, document.getElementById('bca_college').value, document.getElementById('bca_all_sgpa').textContent, document.getElementById('bca_all_pct').textContent, document.getElementById('bca_all_sems').textContent, document.getElementById('bca_all_risk').textContent, document.querySelectorAll('#bca_sem_cards .sem-card'));
}
...
function saveBBASinglePDF() {
  ...
  printPDF('BBA', name, semester, status, risk, subjectRows, document.getElementById('bba_res_total').textContent, document.getElementById('bba_res_pct').textContent, document.getElementById('bba_res_cgpa').textContent, document.getElementById('bba_roll').value, document.getElementById('bba_college').value, document.getElementById('bba_res_weak').textContent);
}

function saveBBAAllPDF() {
  printAllPDF('BBA', document.getElementById('bba_all_res_name').textContent, document.getElementById('bba_roll').value, document.getElementById('bba_college').value, document.getElementById('bba_all_sgpa').textContent, document.getElementById('bba_all_pct').textContent, document.getElementById('bba_all_sems').textContent, document.getElementById('bba_all_risk').textContent, document.querySelectorAll('#bba_sem_cards .sem-card'));
}
...
function saveBComSinglePDF() {
  ...
  printPDF('BCom', name, semester, status, risk, subjectRows, document.getElementById('bcom_res_total').textContent, document.getElementById('bcom_res_pct').textContent, document.getElementById('bcom_res_cgpa').textContent, document.getElementById('bcom_roll').value, document.getElementById('bcom_college').value, document.getElementById('bcom_res_weak').textContent);
}

function saveBComAllPDF() {
  printAllPDF('BCom', document.getElementById('bcom_all_res_name').textContent, document.getElementById('bcom_roll').value, document.getElementById('bcom_college').value, document.getElementById('bcom_all_sgpa').textContent, document.getElementById('bcom_all_pct').textContent, document.getElementById('bcom_all_sems').textContent, document.getElementById('bcom_all_risk').textContent, document.querySelectorAll('#bcom_sem_cards .sem-card'));
}
...
function saveBScSinglePDF() {
  ...
  printPDF('B.Sc', name, semester, status, risk, subjectRows, document.getElementById('bsc_res_total').textContent, document.getElementById('bsc_res_pct').textContent, document.getElementById('bsc_res_cgpa').textContent, document.getElementById('bsc_roll').value, document.getElementById('bsc_college').value, document.getElementById('bsc_res_weak').textContent);
}

function saveBScAllPDF() {
  printAllPDF('B.Sc', document.getElementById('bsc_all_res_name').textContent, document.getElementById('bsc_roll').value, document.getElementById('bsc_college').value, document.getElementById('bsc_all_sgpa').textContent, document.getElementById('bsc_all_pct').textContent, document.getElementById('bsc_all_sems').textContent, document.getElementById('bsc_all_risk').textContent, document.querySelectorAll('#bsc_sem_cards .sem-card'));
}
...
function saveMCASinglePDF() {
  ...
  printPDF('MCA', name, semester, status, risk, subjectRows, document.getElementById('mca_res_total').textContent, document.getElementById('mca_res_pct').textContent, document.getElementById('mca_res_cgpa').textContent, document.getElementById('mca_roll').value, document.getElementById('mca_college').value, document.getElementById('mca_res_weak').textContent);
}

function saveMCAAllPDF() {
  printAllPDF('MCA', document.getElementById('mca_all_res_name').textContent, document.getElementById('mca_roll').value, document.getElementById('mca_college').value, document.getElementById('mca_all_sgpa').textContent, document.getElementById('mca_all_pct').textContent, document.getElementById('mca_all_sems').textContent, document.getElementById('mca_all_risk').textContent, document.querySelectorAll('#mca_sem_cards .sem-card'));
}
function saveMBASinglePDF() {
  ...
  printPDF('MBA', name, semester, status, risk, subjectRows, document.getElementById('mba_res_total').textContent, document.getElementById('mba_res_pct').textContent, document.getElementById('mba_res_cgpa').textContent, document.getElementById('mba_roll').value, document.getElementById('mba_college').value, document.getElementById('mba_res_weak').textContent);
}

function saveMBAAllPDF() {
  printAllPDF('MBA', document.getElementById('mba_all_res_name').textContent, document.getElementById('mba_roll').value, document.getElementById('mba_college').value, document.getElementById('mba_all_sgpa').textContent, document.getElementById('mba_all_pct').textContent, document.getElementById('mba_all_sems').textContent, document.getElementById('mba_all_risk').textContent, document.querySelectorAll('#mba_sem_cards .sem-card'));
}

function printPDF(course, name, semester, status, risk, subjectRows, total, pct, cgpa, roll, college, weak) {
  const content = `<html><head><title>${course} Report - ${name}</title><meta charset="UTF-8"><style>body{font-family:Arial,sans-serif;padding:30px;color:#333}.header{background:#000;color:white;padding:20px;text-align:center;margin-bottom:30px;border-radius:10px}h1{margin:0;font-size:1.5rem}h2{color:#1a73e8;border-bottom:2px solid #1a73e8;padding-bottom:8px;margin-top:30px}table{width:100%;border-collapse:collapse;margin:15px 0}td,th{border:1px solid #ddd;padding:12px 16px;text-align:left}th{background:#1a73e8;color:white}tr:nth-child(even){background:#f5f5f5}.pass{color:green;font-weight:bold}.fail{color:red;font-weight:bold}.footer{text-align:center;color:#999;font-size:0.8rem;margin-top:30px}</style></head><body><div class="header"><h1>🎓 ${course} Performance Report - ${semester}</h1></div><h2>Student Details</h2><table><tr><th>Field</th><th>Details</th></tr><tr><td>Student Name</td><td>${name}</td></tr><tr><td>Roll Number</td><td>${roll}</td></tr><tr><td>College</td><td>${college}</td></tr><tr><td>Semester</td><td>${semester}</td></tr><tr><td>Total Marks</td><td>${total}</td></tr><tr><td>Percentage</td><td>${pct}</td></tr><tr><td>CGPA</td><td>${cgpa}</td></tr><tr><td>Status</td><td class="${status.includes('PASS')?'pass':'fail'}">${status}</td></tr><tr><td>Risk Level</td><td>${risk}</td></tr><tr><td>Weak Subjects</td><td>${weak}</td></tr></table><h2>Subject Marks</h2><table><tr><th>Subject</th><th>Marks (out of 100)</th><th>Status</th></tr>${subjectRows}</table><div class="footer"><p>Generated by Student Performance Analysis System | ${new Date().toLocaleDateString()}</p></div></body></html>`;
  const w = window.open('','_blank');
...
function printAllPDF(course, name, roll, college, sgpa, pct, sems, risk, cards) {
  ...
  cards.forEach(card => {
    ...
    semRows += `<tr><td>${h3}</td><td>${total}</td><td>${p}</td><td>${cgpa}</td><td class="${status.includes('PASS')?'pass':'fail'}">${status}</td></tr>`;
  });
  const content = `<html><head><title>${course} All Sems - ${name}</title><meta charset="UTF-8"><style>body{font-family:Arial,sans-serif;padding:30px;color:#333}.header{background:#000;color:white;padding:20px;text-align:center;margin-bottom:30px;border-radius:10px}h1{margin:0;font-size:1.5rem}h2{color:#1a73e8;border-bottom:2px solid #1a73e8;padding-bottom:8px;margin-top:30px}table{width:100%;border-collapse:collapse;margin:15px 0}td,th{border:1px solid #ddd;padding:12px 16px;text-align:left}th{background:#1a73e8;color:white}tr:nth-child(even){background:#f5f5f5}.pass{color:green;font-weight:bold}.fail{color:red;font-weight:bold}.footer{text-align:center;color:#999;font-size:0.8rem;margin-top:30px}</style></head><body><div class="header"><h1>🎓 ${course} All Semesters Report</h1></div><h2>Student Details</h2><table><tr><th>Field</th><th>Details</th></tr><tr><td>Student Name</td><td>${name}</td></tr><tr><td>Roll Number</td><td>${roll}</td></tr><tr><td>College</td><td>${college}</td></tr><tr><td>SGPA</td><td>${sgpa}</td></tr><tr><td>Overall Percentage</td><td>${pct}</td></tr><tr><td>Completed Semesters</td><td>${sems}</td></tr><tr><td>Risk Level</td><td>${risk}</td></tr></table><h2>Semester wise Results</h2><table><tr><th>Semester</th><th>Total</th><th>Percentage</th><th>CGPA</th><th>Status</th></tr>${semRows}</table><div class="footer"><p>Generated by Student Performance Analysis System | ${new Date().toLocaleDateString()}</p></div></body></html>`;
  const w = window.open('','_blank');
  w.document.write(content);
  w.document.close();
  setTimeout(()=>{w.print();},500);
